...
var common_model = require('./model/common_model.js');
 ...
 var server =require('https').createServer({
   key: fs.readFileSync('ludofantasy.key'),
   cert: fs.readFileSync('ludofantasy.crt'),
...
var totalPrivatePlayers = 0;
var totalOnlineGamePlayers = 0;
...
function winLossCoinsDistribution(match){
	...
	for (var i = 0; i < match.players.length; i++) {
		...
		if(match.players[i].isDeductMoney=="No" && match.isFree=="No" ){
			...
			var coinsDeductData={
			   referredBy:match.players[i].referredBy,
	...
	common_model.joinBotsRoomTable(data,function(res){
		if(res.success==1){	
			...
			var playerObj ={
				...
				socketId:res.userId,
				type:res.playerType,
				inactiveTokenCount:inactiveTokens,
				winCount:0,
		...
		if(res.success==1){
			...
			if(match){
				...
				var match = {
					...
					isGameStart:false,
					winnerCount :0,
					startRoundWaiting:res.startRoundTime,
					gameOverTime:5,
					isGameOver:false,
					throwDieTime:res.rollDiceTime,
					constthrowDieTime:res.rollDiceTime,
					botthrowDieTime:res.rollDiceTime,
					...
					totalTokens: inactiveTokens,
					isTournament: res.isTournament,
				};
				match.players.push(playerObj);			
				matches.push(match);		
				io.in(socket.tableId).emit("playerObject",match.players);
				...
				if(isMove=='Yes'){
					if(player.fourToken[data.tokenIndex].status == 'Inactive'){
						if(player.isHold && (data.holdPosition==6 || data.holdPosition==1)){
							if(data.holdPosition==6 && player.diceSixInTurn!=0){
					...
					var movedPos = diceNum;
					if(player.isHold){
						movedPos = Number(data.holdPosition);
					}
					...
					if(!player.isHold && player.holdPosition.length==0){
						if(player.fourToken[data.tokenIndex].status!='Win'){
						...
						} else if(player.fourToken[data.tokenIndex].status=='Win') {
							match.isTokenMove = true;
							match.disableDice = false;
						}
					} else {
						...
						if(player.fourToken[data.tokenIndex].status=='Win') {
							...
							io.in(match.tableId).emit("playerObject",match.players); //temp
						} else {
							if(player.inactiveTokenCount>0 && (player.holdPosition.includes(6) || player.holdPosition.includes(1))) {
								//match.isTokenMove = true;
								match.isReturn = true;
								match.isReturnMove = true;
								...
								/*IF NO TOKEN CAN BE MOVED THEN NEXT TURN*/
								var lastno = 0;
								...
								var passFlag = true;
								for (let j = 0; j < player.holdPosition.length; j++) {
									if(player.holdPosition[j] <= lastno){
										passFlag = false;
									}
								}
								if(passFlag){
		...
		if(player){
			if(player.isturn==true && match.matchDiceNumber==0){
				if(!match.disableDice){
					...
					if(player.inactiveTokenCount == match.totalTokens && diceNumber != 6 && diceNumber != 1){
						if(player.isHold){
							match.isTokenMove= false;
						} else {
							match.isTokenMove= true;						
						}
					} else if(player.inactiveTokenCount == match.totalTokens && player.isHold){
						match.isTokenMove= true;
					} else if(player.inactiveTokenCount < match.totalTokens){
						var lastno = 0;
						for (let i = 0; i < player.fourToken.length; i++) {
							if(player.fourToken[i].status=='Active'){
								var no = 57 - player.fourToken[i].position;
						...
						if(diceNumber==6 && player.inactiveTokenCount > 0){
							...
							if(!player.isHold && lastno < diceNumber){
								match.isTokenMove= true;
								match.isReturn = false;
								match.isReturnMove = false;
							} else if(player.isHold && player.isHoldPositionPlay){
	...
	for (var i = 0; i < matches.length; i++) {
		if(!matches[i].isGameStart){
			...
			if(matches[i].isGameOver==false && matches[i].leftPlayers==1){
				...
				if(matches[i].throwDieTime==matches[i].botthrowDieTime && matches[i].whosTurn=='Bot' && !matches[i].holdBotTurn){
...
function botDiceRollFunction(match){
	...
	if(player && player.playerType=='Bot'){	
		...
		if(randomNum == 6 && !player.isHoldPositionPlay) {
			match.holdBotTurn = true;
			match.matchDiceNumber = 0;
			(function myFirstLoop () {
				setTimeout(function() {
					botDiceRollFunction(match);
				},2000);
			})(1);
			return;
		}
		...
		if(player.holdPosition.length>0){
			checkBotMoveToken(match,player,randomNum,true,isBehindToken,moveTopPosition,randomTime);
		} else {
			setTimeout(function() {
				checkBotMoveToken(match,player,randomNum,false,isBehindToken,moveTopPosition,randomTime)
				match.holdBotTurn = false;
			},randomTime);
		}		
	}	
}

function checkBotMoveToken(match,player,randomNum,holdFlag,isBehindToken,moveTopPosition,randomTime){
	if(!holdFlag){
		...
		if(((match.gameMode=='Classic' && player.inactiveTokenCount==4) || (match.gameMode=='Quick' && player.inactiveTokenCount==2)) && (randomNum == 6 || randomNum == 1)){
			var data ={
				userId:player.userId,
				tableId:player.tableId,
		...
		var i=1;
		var constRandomTime = randomTime;
		(function myLoop (i) {
			if(i!=1) {
				randomTime = randomTime+400;
			}
			setTimeout(function() {
				randomNum = player.holdPosition[0];
				var isKillerToken = isKillerTokenFunction(match,player,randomNum);
				var isSafferToken = isSafferTokenFunction(player,randomNum);
				var isWinnerToken = isWinnerTokenFunction(player,randomNum);
...
function nextTurnFunction(match,pIndex){
	...
	if(match.isReturn==true && match.isReturnMove ==true){
		...
		match.disableDice = false;
		var currentTrn = nextTurnIndex(match,pIndex,0);
	}
	for (let i = 0; i < match.players.length; i++) {
...
// next index test
function nextTurnIndex(match,pIndex,attempt){
	if(match.leftPlayers!=1){
		next = pIndex+1;
	    ...
	    if(attempt==6){
        	next =0;
    	}else if(match.players[next].status != 'Active'){
			nextTurnIndex(match,next,attempt);
		}
		return next;
	}
}
