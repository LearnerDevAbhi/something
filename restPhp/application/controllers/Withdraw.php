...
class Withdraw extends REST_Controller 
{
    ...
    public function addWithdrawAmount_post() 
    {
        ...
        {
           	$userData =$this->Crud_model->GetData("user_details","*","id='".$userId."'","","","","1");
            ...
            if(!empty($userData)){
	            ...
	            // }elseif($userData->coins < $withdrawAmount){
	            ...
	            }elseif($withdrawAmount <= 10 && $paymentType=='bank'){
	                $response = array('status' => FALSE, 'success' => "2",'msg' => "Withdraw amount should be greater than or equal 10");
	            	...
	            	if(!empty($userRecord)){
	            	 $response = array('status' => FALSE, 'success' => "2",'msg' => "Order Id already exists");
		                ...
		                $totalBal = $getData->winWallet - $withdrawAmount;
		                // $totalBal = $getData->coins - $withdrawAmount;
		                ...
		                $getRecord = $this->Crud_model->GetData('user_details','mainWallet,winWallet,coins',$con,'','','','1');
		                $totalCoins = $getRecord->mainWallet + $getRecord->winWallet;
		                $updateCoin = array(
		                	"coins"=>$totalCoins,
		                );
		                $this->Crud_model->SaveData('user_details',$updateCoin,$con);
