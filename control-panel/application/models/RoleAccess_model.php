...
{
    var $column_order = array(null,'al.name','al.email','al.status','al.created',null); 
    var $column_search = array('al.name','al.email','al.status','al.created'); //set column field database for datatable searchable 
    var $order = array('al.id' => 'DESC'); 
    ...
    public function get_submenu_data($value){
        $this->db->select('m.parentId');
        $this->db->where('menuId',$value);
        return $this->db->get('admin_menus m')->row();
    }
