...
{
	var $column_order = array(null,'cdh.tableId','cdh.gameType','cdh.betValue',null); //set column field database for datatable orderable
	var $column_search = array('cdh.tableId','cdh.gameType','cdh.betValue'); //set column field database for datatable searchable 
	...
	public function getExportData($table,$condition=''){
		...
		if(!empty($condition))
			$this->db->where($condition);
		$this->db->group_by('cdh.tableId');
		$this->db->order_by('cdh.coinsDeductHistoryId DESC');
		$query = $this->db->get();
		return $query->result();
	}
