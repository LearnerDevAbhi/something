...
class CompletedRequest_model extends CI_Model
{
    ...
    function getExportData($table,$condition=''){
        $this->db->select('ua.*,u.user_name,u.user_id,u.user_name,u.email_id,u.balance,u.mobile,bd.acc_holderName,bd.accno,bd.ifsc');
        $this->db->from($table); 
        $this->db->join("user_details u","u.id=ua.user_detail_id","left");
        $this->db->join("bank_details bd","bd.user_detail_id=ua.user_detail_id","left");
