...
// }exit();
	...
	{	
		...
		foreach ($getUsers as $getUserData) 
		{
			...
			//$nestedData[] = $getUserData->balance - $getUserData->mainWallet;
	...
	public function ajaxGamePlayed($id)
	{
		...
		foreach ($getgamePlayed as $gamePlayed) 
		{
			...
			if(!empty($gamePlayed->created) && $gamePlayed->created !="0000-00-00 00:00:00"){ $created = date('d M Y', strtotime($gamePlayed->created)); }else{ $created = '0000-00-00 00:00:00'; }
	...
	public function ajaxCompWithDrawal($id)
	{
		...
		foreach ($getCopmWithdrawal as $compWithdrawal) 
		{
			...
			if(!empty($compWithdrawal->created) && $compWithdrawal->created!="0000-00-00 00:00:00"){ $created = date("d M Y",strtotime($compWithdrawal->created)); }else{ $created = 'NA'; }
	...
	public function ajaxCompDeposit($id)
	{
		...
		foreach ($getCompDeposit as $compDeposit) 
		{
			...
			if(!empty($compDeposit->created) && $compDeposit->created!="0000-00-00 00:00:00"){ $created = date("d M Y",strtotime($compDeposit->created)); }else{ $created = 'NA'; }
	...
	public function ajaxBonusList($id)
	{
		...
		foreach ($getUserBonus as $userBonu) 
		{
			...
			if(!empty($userBonu->created) && $userBonu->created!="0000-00-00 00:00:00"){ $created = date("d M Y",strtotime($userBonu->created)); }else{ $created = 'NA'; }
	...
	public function ajaxPlayGameBonusList($id)
	{
		...
		foreach ($getUserBonus as $playGameBonus) 
		{
			...
			if(!empty($playGameBonus->created) && $playGameBonus->created!='0000-00-00 00:00:00'){ $created = date("d M Y",strtotime($playGameBonus->created)); }else{ $created = 'NA'; }
	...
	public function view($id)
	{
		...
		//print_r($getReferralCount->refCount);exit;
		$data = array(
	...
	public function addMoney(){
		...
		//$txnMode= $this->input->post('txnMode');
		$getUserBal = $this->Crud_model->GetData("user_details",'id,user_name,mobile,balance,mainWallet,referredAmt',"id='".$id."'",'','','','1');
		...
		if($addTo=='mainWallet'){
			...
			$type='Admin';
			$updateRefAmt=$getUserBal->referredAmt + $this->input->post('amount');
	...
	public function deductMoney(){
		...
		$getUserBal = $this->Crud_model->GetData("user_details",'id,balance,mainWallet',"id='".$id."'",'','','','1');
		if(!empty($getUserBal->balance) ){
			if($addTo=='mainWallet'){
				// if(!empty($getUserBal->balance) && $deductAmt <= $getUserBal->balance){
					$amt = $getUserBal->balance;
				...
				if($deductAmt <= ($getUserBal->balance-$getUserBal->mainWallet)){ // changed by Piyush as balance calculations were going to negative
