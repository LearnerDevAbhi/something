<?php
  ...
  $withraw_list=$this->Crud_model->multijoin('user_details u',"u.user_name,u.id","rr.amount!='0' and rr.status='Pending' and rr.isReadNotification='No' and rr.type='Withdraw'",'u.id','created','',array('user_account rr'),array('rr.user_detail_id=u.id'),array('left')); 
?>
<!DOCTYPE html>
<html>
<head>
      ...
      <div class="navbar-custom-menu">
        <ul class="nav navbar-nav">
          ...
          <li class="dropdown messages-menu">
            ...
            <a href="javascript:void(0)" class="dropdown-toggle" <?php if(count($withraw_list)){ ?> data-toggle="dropdown"  id="blink" <?php  }  ?> aria-expanded="true" title="Withdraw  Requests">
              <i class="fa fa-money"></i>
              <?php if(count($withraw_list)){ ?> 
              <span class="label label-success"><?= count($withraw_list); ?></span>
              <?php  }  ?>
            </a>
            <?php } ?>
            <ul class="dropdown-menu">
              <li class="header">You have <?= count($withraw_list); ?> Withdraw Requests</li> 
              <li>
                <!-- inner menu: contains the actual data -->
                <ul class="menu">  

                <?php  foreach ($withraw_list as $withraw) { ?>
                  <li><!-- start message -->
                    <a href="<?= site_url(WITHDRAWAL); ?>">
                      <h4 style="margin-left:0px;color:red; float: left;">
                       <?= $withraw->user_name; ?>
                      </h4> 
                      <span class="pull-right">From User</span>
                    </a>  
                  </li>
                <?php  }  ?>
                </ul>
              ...
              <li class="footer">
                <!-- <form action="" method="post">  -->
                   <!-- <button type='submit' class='clr-button'>See All Withdraw Requests</button>     -->
                   <a href="">See All Withdraw  Requests</a>  
               <!--  </form> -->
              </li>
            </ul>
          </li>
